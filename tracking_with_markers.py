...
def draw_marker_with_angle(imaxis, imagePoints):
	points_im = np.float32([[imagePoints[0][0][0],imagePoints[0][0][1]], [imagePoints[2][0][0], imagePoints[2][0][1]]])
	rows,cols = imaxis.shape[:2]
	[vx,vy,x,y] = cv2.fitLine(points_im, cv2.DIST_L2,0,0.01,0.01)
	lefty = int((-x*vy/vx) + y)
	righty = int(((cols-x)*vy/vx)+y)
	# cv2.line(imaxis,(cols-1,righty),(10,lefty),(0,255,0),2)
	if imagePoints[0][0][0] - imagePoints[2][0][0] > 0:
		angle = int(math.atan((points_im[1][1]-points_im[0][1])/(points_im[0][0]-points_im[1][0]))*180/math.pi)
	else: angle = 180 - int(math.atan((points_im[1][1]-points_im[0][1])/(points_im[1][0]-points_im[0][0]))*180/math.pi)
	return angle


def find_motion(x,y):
	if (x[0]-y[0])*(x[0]-y[0])+(x[1]-y[1])*(x[1]-y[1]) > 500 :
		return True
	else:
		return False

def print_warped(cv_image, warped, id, pts, angle, overlay_1, overlay_2):
	angle = math.radians(angle)
	# print("id:", id, "angle=", angle, "cos, sin", math.cos(angle), math.sin(angle))
	desired_size = 50
	...
	color = [0, 0, 0]
	warped_1 = cv2.copyMakeBorder(warped, top, bottom, left, right, cv2.BORDER_CONSTANT,
	    value=color)
	...
	if overlay_1 > 0:
		if overlay_2 > 0:
			cv2.putText(cv_image, "id: {}".format(overlay_1), (center[0]-25, center[1]+40),  cv2.FONT_HERSHEY_SIMPLEX, 0.7,
				(0, 0, 255), 2)
			cv2.putText(cv_image, "id: {}".format(overlay_2), (center[0]-25, center[1]+25),  cv2.FONT_HERSHEY_SIMPLEX, 0.7,
				(0, 0, 255), 2)
		else: cv2.putText(cv_image, "id: {}".format(overlay_1), (center[0]-25, center[1]+25),  cv2.FONT_HERSHEY_SIMPLEX, 0.7,
				(0, 0, 255), 2)

	cv2.putText(cv_image, "id: {}".format(id), (center[0]-25, center[1]+5),  cv2.FONT_HERSHEY_SIMPLEX, 0.7,
		(0, 0, 255), 2)
	if id != -1 :
...
def print_all_markers(cv_image, height, width, ids, i, id_base_1, id_base_2, id_base_3, overlay_base_1, overlay_base_2, overlay_2_marker, photos, last_motion_photos):
	photos_1 = []
	photos_2 = []
	photos_3 = []
	overlay_2 = 0
			...
			if photos_2[j][1] not in overlay_base_2:
				for k in range(len(overlay_2_marker)):
					if photos_2[j][1] == overlay_2_marker[k][1]:
						print_warped(cv_image, photos_2[j][0], photos_2[j][1], photos_2[j][2], photos_2[j][3], overlay_2_marker[k][0], 0)
		for j in range(len(id_base_3)):	
			for k in range(len(overlay_2_marker)):
				if photos_3[j][1] == overlay_2_marker[k][1]:
					...
					print_warped(cv_image, photos_3[j][0], photos_3[j][1], photos_3[j][2], photos_3[j][3], overlay_2[0], overlay_2_marker[k][0])
		photos = photos_1+photos_2+photos_3
	return photos

filename = 'C:\\Users\\ct\\Downloads\\Telegram Desktop\\map_room_29.mp4'
...
with open("C:\\Users\\ct\\Downloads\\Telegram Desktop\\calibration_7.yaml") as f:
...
while True:
	...
	for i in range(len(ids)):
		...
		cv2.line(imaxis, (imagePoints[0][0][0],imagePoints[0][0][1]), (imagePoints[2][0][0], imagePoints[2][0][1]), (0, 255, 0), 3)
		angle = draw_marker_with_angle(imaxis, imagePoints)

		id_marker = ids[i][0]
		marker_frames[id_marker]+=1
		pts = corners[i][0]
		warped = four_point_transform(image, pts)
		center_of_marker[id_marker][marker_frames[id_marker]] = (int((pts[0][0]+pts[2][0])/2), int((pts[0][1]+pts[2][1])/2))
		...
		if id_marker not in id_base :
			...
			# print_warped(cv_image, photos_1[ids], photos[-1][1], photos[-1][2])
			# print_warped(cv_image, warped, id_motion, pts, 0)
		# print("check3: ", check_overlay)
		if marker_frames[id_marker] > 2:
			motion = find_motion(center_of_marker[id_marker][marker_frames[id_marker]], center_of_marker[id_marker][marker_frames[id_marker]-2])
		if motion:
			motion = False
			...
			if id_marker != id_motion :
				...
				check_overlay[:] = 0
				frames_1 = 0
			photos = print_all_markers(cv_image, height, width, ids, i, id_base_1, id_base_2, id_base_3, overlay_base_1, overlay_base_2, overlay_2_marker, photos, last_motion_photos)
	...
	for i in range(len(id_base_1)):
		if draw_after_motion > 2 and frames_1 == 0 and marker_frames[int(id_base_1[i])] - check_overlay[int(id_base_1[i])] == 0  and counter_overlay > len(id_base)+2 and id_base_1[i] not in overlay_base_1 :
			# print()
			frames_1+=1
	...
	for i in range(len(id_base_2)):
		if draw_after_motion > 2 and frames_1 == 0 and marker_frames[int(id_base_2[i])] - check_overlay[int(id_base_2[i])] == 0  and counter_overlay > len(id_base)+2 and id_base_2[i] not in overlay_base_2 :
			# print()
			frames_1+=1
	...
	if writer is None:
		...
		writer = cv2.VideoWriter("C:\\Users\\ct\\motion_detect_1026.avi", fourcc, 15,
